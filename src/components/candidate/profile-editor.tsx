  ...
  return (
    <div className="space-y-3 rounded-lg border bg-muted/20 p-4">
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <div className="space-y-1">
          <Label className="text-xs">{labels.position}</Label>
          <Input
            value={data.position}
        ...
        <div className="space-y-1 flex items-end gap-3">
          <label className="flex items-center gap-2 text-sm pb-2">
            <input
              ...
              onChange={(e) => onChange({ ...data, isCurrent: e.target.checked })}
            />
            {labels.currentRole}
          </label>
        </div>
      ...
      <div className="space-y-1">
        <Label className="text-xs">{labels.technologies}</Label>
        <Input
...
export function ProfileEditor({
  ...
  const [editProject, setEditProject] = useState<
    Record<string, typeof emptyProject>
  >({});
  ...
  const [editLanguage, setEditLanguage] = useState<
    Record<
      string,
      { name: string; proficiency: (typeof proficiencyOptions)[number] }
    >
  >({});
  ...
  async function handleImport(file: File) {
    ...
    try {
      const res = await fetch("/api/candidate/profile/import", {
        method: "POST",
        body: formData,
      });
      const json = await res.json();
  ...
  async function addSkill() {
    ...
    try {
      await apiCall("/api/candidate/profile/skills", "POST", {
        name: newSkill.trim(),
      });
      flash(m.skillAdded);
      setNewSkill("");
  ...
  const expFormLabels = {
    ...
    endDate: p.endDate,
    bullets: p.bullets,
    bulletsPlaceholder: p.bulletsPlaceholder,
  ...
  return (
    <div className="space-y-6">
      {(error || success) && (
        <div
          className={`rounded-md p-3 text-sm ${error ? "bg-destructive/10 text-destructive" : "bg-success/10 text-success"}`}
        >
          {error || success}
        </div>
      )}
          ...
          {editingContact ? (
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="space-y-1 sm:col-span-2">
                ...
                <Input
                  value={contactForm.professionalTitle}
                  onChange={(e) =>
                    setContactForm({
                      ...contactForm,
              ...
              <div className="space-y-1">
                <Label className="text-xs">{p.phone}</Label>
                <Input
              ...
              <div className="space-y-1">
                <Label className="text-xs">{p.linkedIn}</Label>
                <Input
              ...
              <div className="space-y-1">
                <Label className="text-xs">{p.github}</Label>
                <Input
              ...
              <div className="space-y-1">
                <Label className="text-xs">{p.portfolio}</Label>
                <Input
              ...
              <div className="space-y-1">
                <Label className="text-xs">{p.city}</Label>
                <Input
              ...
              <div className="space-y-1">
                <Label className="text-xs">{p.country}</Label>
                <Input
              ...
              <div className="space-y-1 sm:col-span-2">
                ...
                <Input
                  value={contactForm.technologies}
                  onChange={(e) =>
                    setContactForm({
                      ...contactForm,
            ...
            <div className="grid gap-2 sm:grid-cols-2 text-sm">
              <div>
                <span className="text-muted-foreground">{p.titleLabel}: </span>
                {passport.professionalTitle || t.common.empty}
              </div>
              <div>
                <span className="text-muted-foreground">{p.emailLabel}: </span>
                {email}
              </div>
              <div>
                <span className="text-muted-foreground">{p.phone}: </span>
                {passport.phone || t.common.empty}
              </div>
              <div>
                <span className="text-muted-foreground">{p.linkedIn}: </span>
                {passport.linkedIn || t.common.empty}
              </div>
              <div>
                <span className="text-muted-foreground">{p.github}: </span>
                {passport.github || t.common.empty}
              </div>
              <div>
                <span className="text-muted-foreground">{p.portfolio}: </span>
                {passport.portfolio || t.common.empty}
              </div>
              <div>
                <span className="text-muted-foreground">{p.city}: </span>
                {passport.city || t.common.empty}
              </div>
              <div>
                <span className="text-muted-foreground">{p.country}: </span>
                {passport.country || t.common.empty}
              </div>
            </div>
          )}
        </CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{p.autoSummaryTitle}</CardTitle>
          <CardDescription>{p.autoSummaryDesc}</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle>
            {p.experience} ({passport.experiences.length})
          </CardTitle>
          <Button
            size="sm"
            variant="outline"
              ...
              <div className="flex items-start justify-between p-4">
                ...
                >
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    {formatMonth(exp.startDate, locale)} –{" "}
                    {exp.isCurrent
                      ? t.resume.present
                      : formatMonth(exp.endDate, locale)}
                  </p>
                </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle>
            {p.skills} ({passport.skills.length})
          </CardTitle>
          <Button
            size="sm"
            variant="outline"
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle>
            {p.education} ({passport.education.length})
          </CardTitle>
          <Button
            size="sm"
            variant="outline"
        ...
        <CardContent className="space-y-4">
          {addingEdu && (
            <div className="space-y-3 rounded-lg border bg-muted/20 p-4">
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="space-y-1">
                  <Label className="text-xs">{p.institution}</Label>
                  <Input
                ...
                <div className="space-y-1">
                  <Label className="text-xs">{p.degree}</Label>
                  <Input
                    value={newEdu.degree}
                    onChange={(e) =>
                ...
                <div className="space-y-1">
                  <Label className="text-xs">{p.end}</Label>
                  <Input
                    type="month"
              ...
              <div className="flex gap-2">
                <Button size="sm" onClick={addEducation} disabled={saving}>
                  {t.common.add}
                </Button>
                <Button
                  size="sm"
                ...
                <>
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className="space-y-1">
                      <Label className="text-xs">{p.institution}</Label>
                      <Input
                        value={editEdu[edu.id].institution}
                        onChange={(e) =>
                          setEditEdu({
                            ...editEdu,
                            [edu.id]: {
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        type="month"
                        value={editEdu[edu.id].startDate}
                        onChange={(e) =>
                          setEditEdu({
                            ...editEdu,
                            [edu.id]: {
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" onClick={() => saveEducation(edu.id)}>
                      {t.common.save}
                    </Button>
                    <Button
                      size="sm"
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {formatMonth(edu.startDate, locale)} –{" "}
                      {formatMonth(edu.endDate, locale)}
                    </p>
                  </div>
                  <div className="flex gap-1">
                    <Button
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle>
            {p.projectsTitle} ({passport.projects.length})
          </CardTitle>
          <Button
            size="sm"
            variant="outline"
        ...
        <CardContent className="space-y-3">
          {addingProject && (
            <div className="space-y-3 rounded-lg border bg-muted/20 p-4">
              <div className="space-y-1">
                <Label className="text-xs">{p.projectTitle}</Label>
                <Input
              ...
              <div className="space-y-1">
                <Label className="text-xs">{p.projectDescription}</Label>
                <textarea
              ...
              <div className="space-y-1">
                <Label className="text-xs">{p.projectTechnologies}</Label>
                <Input
              ...
              {editProject[project.id] ? (
                <>
                  <div className="space-y-1">
                    <Label className="text-xs">{p.projectTitle}</Label>
                    <Input
                      value={editProject[project.id].title}
                      onChange={(e) =>
                        setEditProject({
                          ...editProject,
                          [project.id]: {
                  ...
                  <div className="space-y-1">
                    ...
                    <textarea
                      className="flex min-h-[80px] w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                      value={editProject[project.id].description}
                      onChange={(e) =>
                        setEditProject({
                          ...editProject,
                          [project.id]: {
                  ...
                  <div className="space-y-1">
                    <Label className="text-xs">{p.projectTechnologies}</Label>
                    <Input
                      value={editProject[project.id].technologies}
                      onChange={(e) =>
                        setEditProject({
                          ...editProject,
                          [project.id]: {
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" onClick={() => saveProject(project.id)}>
                      {t.common.save}
                    </Button>
                    <Button
                      size="sm"
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle>
            {p.languagesTitle} ({passport.languages.length})
          </CardTitle>
          <Button
            size="sm"
            variant="outline"
        ...
        <CardContent className="space-y-3">
          {addingLanguage && (
            <div className="flex flex-wrap gap-2 items-end">
              <div className="space-y-1">
                <Label className="text-xs">{p.languageName}</Label>
                <Input
                  ...
                  onChange={(e) =>
                    setNewLanguage({
                      ...
                      proficiency: e.target
                        .value as typeof newLanguage.proficiency,
                    })
                  }
                >
              ...
              <Button
                size="sm"
                onClick={addLanguage}
                disabled={saving || !newLanguage.name.trim()}
              >
                {t.common.add}
              </Button>
              <Button
            ...
            >
              {editLanguage[lang.id] ? (
                <div className="flex flex-wrap gap-2 items-end flex-1">
                  <Input
                    value={editLanguage[lang.id].name}
                    onChange={(e) =>
                      setEditLanguage({
                        ...editLanguage,
                        [lang.id]: {
                  ...
                  <select
                    className="h-9 rounded-md border border-input bg-background px-3 text-sm"
                    value={editLanguage[lang.id].proficiency}
                    onChange={(e) =>
                      setEditLanguage({
                        ...editLanguage,
                        [lang.id]: {
                          ...editLanguage[lang.id],
                          proficiency: e.target
                            .value as (typeof proficiencyOptions)[number],
                        },
                      })
                    }
                  >
                  ...
                  <Button size="sm" onClick={() => saveLanguage(lang.id)}>
                    {t.common.save}
                  </Button>
                  <Button
                    size="sm"
                      ...
                      onClick={() =>
                        setEditLanguage({
                          ...
                          [lang.id]: {
                            name: lang.name,
                            proficiency:
                              lang.proficiency as (typeof proficiencyOptions)[number],
                          },
                        })
                      }
                    >
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle>
            {p.certifications} ({passport.certifications.length})
          </CardTitle>
          <Button
            size="sm"
            variant="outline"
        ...
        <CardContent className="space-y-3">
          {addingCert && (
            <div className="flex flex-wrap gap-2 items-end">
              <div className="space-y-1">
                <Label className="text-xs">{p.certName}</Label>
                <Input
                  value={newCert.name}
                  onChange={(e) =>
              ...
              <div className="space-y-1">
                <Label className="text-xs">{p.certIssuer}</Label>
                <Input
              ...
              {editCert[cert.id] ? (
                <div className="flex flex-wrap gap-2 items-end flex-1">
                  ...
                  <Input
                    ...
                    className="max-w-xs"
                  />
                  <Button size="sm" onClick={() => saveCert(cert.id)}>
                    {t.common.save}
                  </Button>
                  <Button
                    size="sm"
                  ...
                  <div className="flex gap-1">
                    <Button
                      ...
                      onClick={() =>
                        setEditCert({
                          ...editCert,
                          [cert.id]: { name: cert.name, issuer: cert.issuer },
                        })
                      }
                    >
                      <Pencil className="h-4 w-4" />
